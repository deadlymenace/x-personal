/**
 * Format tweets for display output.
...
export function formatTweetTelegram(
  ...
  const engagement = `${compactNumber(t.metrics.likes)} likes ${compactNumber(t.metrics.impressions)} views`;
  const time = timeAgo(t.created_at);

  const text =
    opts?.full || t.text.length <= 200
      ? t.text
      : t.text.slice(0, 197) + "...";
